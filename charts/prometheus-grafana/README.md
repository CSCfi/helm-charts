...
This Helm Chart helps you to deploy Grafana and Prometheus on CSC Rahti or Lumi-K (Openshift 4).
...
By default, the Grafana hostname will be generated by default. You can set your own in:

```yaml
route:
  grafana:
    enabled: true
    host: "" # <-- Edit this value. For example my-grafana.rahtiapp.fi or my-grafana.apps.lumi-k.eu
```

If you decide to deploy the Prometheus Route, you can set your own hostname:

```yaml
route:
  prometheus:
    enabled: true
    host: "" # <-- Edit this value. For example my-prometheus.rahtiapp.fi or my-prometheus.apps.lumi-k.eu
```
...
```sh
helm upgrade --install graf-prom . -f values.yaml
```
